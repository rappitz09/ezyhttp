...
from ezyhttp.hdrs import CONTENT_LENGTH, CONTENT_TYPE, TRANSFER_ENCODING
from ezyhttp.test_utils import make_mocked_coro
from ezyhttp.typedefs import Handler
...
async def test_simple_get(ezyhttp_client: Any) -> None:
...
async def test_simple_get_with_text(ezyhttp_client: Any) -> None:
    async def handler(request):
        body = await request.read()
        assert b"" == body
...
async def test_handler_returns_not_response(
    ezyhttp_server: Any, ezyhttp_client: Any
) -> None:
    ...
    app = web.Application()
    app.router.add_get("/", handler)
    server = await ezyhttp_server(app, logger=logger)
    client = await ezyhttp_client(server)
...
async def test_handler_returns_none(ezyhttp_server: Any, ezyhttp_client: Any) -> None:
    asyncio.get_event_loop().set_debug(True)
    logger = mock.Mock()
...
async def test_head_returns_empty_body(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response(body=b"test")

    app = web.Application()
    app.router.add_head("/", handler)
    client = await ezyhttp_client(app, version=HttpVersion11)
...
async def test_response_before_complete(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response(body=b"OK")
...
async def test_post_form(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_post_text(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_post_json(ezyhttp_client: Any) -> None:
    dct = {"key": "текст"}
...
async def test_multipart(ezyhttp_client: Any) -> None:
...
async def test_multipart_empty(ezyhttp_client: Any) -> None:
    with multipart.MultipartWriter() as writer:
        pass
...
async def test_multipart_content_transfer_encoding(ezyhttp_client: Any) -> None:
    # For issue #1168
    with multipart.MultipartWriter() as writer:
...
async def test_render_redirect(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_post_single_file(ezyhttp_client: Any) -> None:
...
async def test_files_upload_with_same_key(ezyhttp_client: Any) -> None:
    async def handler(request):
        data = await request.post()
...
async def test_post_files(ezyhttp_client: Any) -> None:
    here = pathlib.Path(__file__).parent
...
async def test_release_post_data(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_POST_DATA_with_content_transfer_encoding(ezyhttp_client: Any) -> None:
    async def handler(request):
        data = await request.post()
...
async def test_post_form_with_duplicate_keys(ezyhttp_client: Any) -> None:
    async def handler(request):
        data = await request.post()
        lst = list(data.items())
        assert [("a", "1"), ("a", "2")] == lst
...
async def test_expect_default_handler_unknown(ezyhttp_client: Any) -> None:
...
async def test_100_continue(ezyhttp_client: Any) -> None:
    async def handler(request):
        data = await request.post()
        assert b"123" == data["name"]
        return web.Response()
...
async def test_100_continue_custom(ezyhttp_client: Any) -> None:
...
async def test_100_continue_custom_response(ezyhttp_client: Any) -> None:
    async def handler(request):
        data = await request.post()
        assert b"123", data["name"]
        return web.Response()
...
async def test_expect_handler_custom_response(ezyhttp_client: Any) -> None:
    cache = {"foo": "bar"}
...
async def test_100_continue_for_not_found(ezyhttp_client: Any) -> None:
    app = web.Application()
    client = await ezyhttp_client(app)
...
async def test_100_continue_for_not_allowed(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response()
...
async def test_http11_keep_alive_default(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response()
...
@pytest.mark.xfail
async def test_http10_keep_alive_default(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response()

    app = web.Application()
    app.router.add_get("/", handler)
    client = await ezyhttp_client(app, version=HttpVersion10)
...
async def test_http10_keep_alive_with_headers_close(ezyhttp_client: Any) -> None:
    async def handler(request):
        await request.read()
        return web.Response(body=b"OK")
...
async def test_http10_keep_alive_with_headers(ezyhttp_client: Any) -> None:
    async def handler(request):
        await request.read()
        return web.Response(body=b"OK")
...
async def test_upload_file(ezyhttp_client: Any) -> None:
    here = pathlib.Path(__file__).parent
    fname = here / "ezyhttp.png"
...
async def test_upload_file_object(ezyhttp_client: Any) -> None:
    here = pathlib.Path(__file__).parent
    fname = here / "ezyhttp.png"
    with fname.open("rb") as f:
        data = f.read()
...
async def test_empty_content_for_query_with_body(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_get_with_empty_arg(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_large_header(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response()
...
async def test_large_header_allowed(ezyhttp_client: Any, ezyhttp_server: Any) -> None:
    async def handler(request):
        return web.Response()

    app = web.Application()
    app.router.add_post("/", handler)
    server = await ezyhttp_server(app, max_field_size=81920)
    client = await ezyhttp_client(server)
...
async def test_get_with_empty_arg_with_equal(ezyhttp_client: Any) -> None:
    async def handler(request):
        assert "arg" in request.query
        assert "" == request.query["arg"]
        return web.Response()
...
async def test_response_with_async_gen(ezyhttp_client: Any, fname: Any) -> None:
    with fname.open("rb") as f:
        data = f.read()
...
async def test_response_with_file(ezyhttp_client: Any, fname: Any) -> None:
...
async def test_response_with_file_ctype(ezyhttp_client: Any, fname: Any) -> None:
    outer_file_descriptor = None
...
async def test_response_with_payload_disp(ezyhttp_client: Any, fname: Any) -> None:
    outer_file_descriptor = None
    ...
    async def handler(request):
        nonlocal outer_file_descriptor
        outer_file_descriptor = fname.open("rb")
        pl = ezyhttp.get_payload(outer_file_descriptor)
...
async def test_response_with_payload_stringio(ezyhttp_client: Any, fname: Any) -> None:
    async def handler(request):
...
async def test_response_with_precompressed_body(
    ezyhttp_client: Any, compressor: Any, encoding: Any
) -> None:
    async def handler(request):
...
async def test_response_with_precompressed_body_brotli(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_bad_request_payload(ezyhttp_client: Any) -> None:
    async def handler(request):
        assert request.method == "POST"

        with pytest.raises(ezyhttp.web.RequestPayloadError):
...
async def test_stream_response_multiple_chunks(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_start_without_routes(ezyhttp_client: Any) -> None:
    app = web.Application()
    client = await ezyhttp_client(app)
...
async def test_requests_count(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response()
...
async def test_redirect_url(ezyhttp_client: Any) -> None:
...
async def test_simple_subapp(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_subapp_reverse_url(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_subapp_reverse_variable_url(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_subapp_reverse_static_url(ezyhttp_client: Any) -> None:
    fname = "ezyhttp.png"
...
async def test_subapp_app(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_subapp_not_found(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response(text="OK")
...
async def test_subapp_not_found2(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response(text="OK")
...
async def test_subapp_not_allowed(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response(text="OK")
...
async def test_subapp_cannot_add_app_in_handler(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_old_style_subapp_middlewares(ezyhttp_client: Any) -> None:
    order = []
...
async def test_subapp_on_response_prepare(ezyhttp_client: Any) -> None:
    order = []
...
async def test_subapp_on_startup(ezyhttp_server: Any) -> None:
    order = []
    ...
    await ezyhttp_server(app)

    assert [app, subapp1, subapp2] == order


async def test_subapp_on_shutdown(ezyhttp_server: Any) -> None:
    order = []
...
async def test_subapp_on_cleanup(ezyhttp_server: Any) -> None:
    order = []
...
async def test_subapp_middleware_context(
    ezyhttp_client: Any, route: Any, expected: Any, middlewares: Any
):
    values = []
...
async def test_custom_date_header(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_response_prepared_with_clone(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_app_max_client_size(ezyhttp_client: Any) -> None:
    async def handler(request):
        await request.post()
        return web.Response(body=b"ok")
...
async def test_app_max_client_size_adjusted(ezyhttp_client: Any) -> None:
    async def handler(request):
        await request.post()
        return web.Response(body=b"ok")
...
async def test_app_max_client_size_none(ezyhttp_client: Any) -> None:
    async def handler(request):
        await request.post()
        return web.Response(body=b"ok")
...
async def test_post_max_client_size(ezyhttp_client: Any) -> None:
    async def handler(request):
        await request.post()
        return web.Response()
...
async def test_post_max_client_size_for_file(ezyhttp_client: Any) -> None:
    async def handler(request):
        await request.post()
        return web.Response()
...
async def test_response_with_bodypart(ezyhttp_client: Any) -> None:
    async def handler(request):
        reader = await request.multipart()
        part = await reader.next()
...
async def test_response_with_bodypart_named(ezyhttp_client: Any, tmp_path: Any) -> None:
    async def handler(request):
        reader = await request.multipart()
        part = await reader.next()
        return web.Response(body=part)
...
async def test_response_with_bodypart_invalid_name(ezyhttp_client: Any) -> None:
    async def handler(request):
        reader = await request.multipart()
        part = await reader.next()
        return web.Response(body=part)
    ...
    with ezyhttp.MultipartWriter() as mpwriter:
...
async def test_request_clone(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_await(ezyhttp_server: Any) -> None:
    async def handler(request):
...
async def test_response_context_manager(ezyhttp_server: Any) -> None:
    async def handler(request):
        return web.Response()
...
async def test_response_context_manager_error(ezyhttp_server: Any) -> None:
    async def handler(request):
...
async def ezyhttp_client_api_context_manager(ezyhttp_server: Any):
    async def handler(request):
        return web.Response()
...
async def test_iter_any(ezyhttp_server: Any) -> None:
...
async def test_request_tracing(ezyhttp_server: Any) -> None:
    ...
    class FakeResolver:
        ...
        def __init__(self, fakes):
            # fakes -- dns -> port dict
            self._fakes = fakes
            self._resolver = ezyhttp.DefaultResolver()
    ...
    connector = ezyhttp.TCPConnector(resolver=resolver)
    client = ezyhttp.ClientSession(connector=connector, trace_configs=[trace_config])
...
async def test_raise_http_exception(ezyhttp_client: Any) -> None:
    async def handler(request):
        raise web.HTTPForbidden()
...
async def test_request_path(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_app_add_routes(ezyhttp_client: Any) -> None:
    async def handler(request):
        return web.Response()
...
async def test_request_headers_type(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_signal_on_error_handler(ezyhttp_client: Any) -> None:
...
@pytest.mark.skipif(
    "HttpRequestParserC" not in dir(ezyhttp.http_parser),
    reason="C based HTTP parser not available",
)
async def test_bad_method_for_c_http_parser_not_hangs(ezyhttp_client: Any) -> None:
    app = web.Application()
...
async def test_read_bufsize(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_response_101_204_no_content_length_http11(
    ...
    app = web.Application()
    app.router.add_get("/", handler)
    client = await ezyhttp_client(app, version="1.1")
    resp = await client.get("/")
...
async def test_stream_response_headers_204(ezyhttp_client: Any):
    async def handler(_):
...
async def test_httpfound_cookies_302(ezyhttp_client: Any) -> None:
    async def handler(_):
        resp = web.HTTPFound("/")
...
async def test_no_body_for_1xx_204_304_responses(
    ezyhttp_client: Any, status: int, version: HttpVersion
) -> None:
    ...
    app = web.Application()
    app.router.add_get("/", handler)
    client = await ezyhttp_client(app, version=version)
    resp = await client.get("/")
    assert CONTENT_TYPE not in resp.headers
    assert TRANSFER_ENCODING not in resp.headers
    await resp.read() == b""
