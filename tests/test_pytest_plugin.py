...
from ezyhttp import pytest_plugin

pytest_plugins: str = "pytester"

CONFTEST: str = """
pytest_plugins = 'ezyhttp.pytest_plugin'
"""
...
def test_ezyhttp_plugin(testdir: Any) -> None:
    testdir.makepyfile(
        """\
import pytest
...
async def test_hello(ezyhttp_client) -> None:
    client = await ezyhttp_client(await create_app())
...
async def test_hello_from_app(ezyhttp_client) -> None:
    app = web.Application()
...
async def test_hello_with_loop(ezyhttp_client) -> None:
    client = await ezyhttp_client(await create_app())
    resp = await client.get('/')
    assert resp.status == 200
    text = await resp.text()
...
def cli(loop, ezyhttp_client):
    return loop.run_until_complete(ezyhttp_client(create_stateful_app()))
...
async def test_failed_to_create_client(ezyhttp_client) -> None:

    def make_app():
        raise RuntimeError()

    with pytest.raises(RuntimeError):
        await ezyhttp_client(make_app())


async def test_custom_port_ezyhttp_client(ezyhttp_client, ezyhttp_unused_port):
...
async def test_custom_port_test_server(ezyhttp_server, ezyhttp_unused_port):
    app = await create_app()
    port = ezyhttp_unused_port()
    server = await ezyhttp_server(app, port=port)
    assert server.port == port

"""
    )
    testdir.makeconftest(CONFTEST)
    result = testdir.runpytest("-p", "no:sugar", "--ezyhttp-loop=pyloop")
...
"""
    ...
    result = testdir.runpytest(
        "-p", "no:sugar", "-s", "-W", "default", "--ezyhttp-loop=pyloop"
    )
    expected_outcomes = (
        {"failed": 0, "passed": 2}
...
def test_ezyhttp_plugin_async_fixture(testdir: Any, capsys: Any) -> None:
    testdir.makepyfile(
        """\
import pytest
...
@pytest.fixture
async def cli(ezyhttp_client, loop):
    client = await ezyhttp_client(create_app())
    return client
...
def test_ezyhttp_plugin_async_gen_fixture(testdir: Any) -> None:
    testdir.makepyfile(
        """\
import pytest
from unittest import mock
...
@pytest.fixture
async def cli(ezyhttp_client, loop):
    yield await ezyhttp_client(create_app())
    canary()
...
def test_ezyhttp_client_cls_fixture_custom_client_used(testdir: Any) -> None:
    testdir.makepyfile(
        """
import pytest
from ezyhttp.web import Application
from ezyhttp.test_utils import TestClient
...
async def test_hello(ezyhttp_client) -> None:
    client = await ezyhttp_client(Application())
...
def test_ezyhttp_client_cls_fixture_factory(testdir: Any) -> None:
