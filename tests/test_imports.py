...
def test___all__(pytester: pytest.Pytester) -> None:
    """See https://github.com/rappitz09/ezyhttp/issues/6197"""
    pytester.makepyfile(
        test_a="""
            from ezyhttp import *
...
def test_import_time(pytester: pytest.Pytester) -> None:
    """Check that importing ezyhttp doesn't take too long.
    ...
    best_time_ms = 1000
    cmd = "import timeit; print(int(timeit.timeit('import ezyhttp', number=1) * 1000))"
    try:
        for _ in range(3):
