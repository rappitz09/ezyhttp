...
import ezyhttp
from ezyhttp.client_reqrep import ClientRequest, ClientResponse
from ezyhttp.helpers import TimerNoop
...
class TestProxy(unittest.TestCase):
    ...
    @mock.patch("ezyhttp.connector.ClientRequest")
    def test_connect(self, ClientRequestMock: Any) -> None:
        ...
        conn = self.loop.run_until_complete(
            connector.connect(req, None, ezyhttp.ClientTimeout())
        )
    ...
    def test_proxy_dns_error(self) -> None:
        ...
        with self.assertRaises(ezyhttp.ClientConnectorError):
    ...
    def test_proxy_connection_error(self) -> None:
        ...
        with self.assertRaises(ezyhttp.ClientProxyConnectionError):
            self.loop.run_until_complete(
                connector.connect(req, None, ezyhttp.ClientTimeout())
            )
    ...
    def test_proxy_server_hostname_default(self, ClientRequestMock) -> None:
        ...
        self.loop.run_until_complete(
            connector._create_connection(req, None, ezyhttp.ClientTimeout())
        )
    ...
    def test_https_connect_certificate_error(self, ClientRequestMock: Any) -> None:
        ...
        with self.assertRaises(ezyhttp.ClientConnectorCertificateError):
            self.loop.run_until_complete(
                connector._create_connection(req, None, ezyhttp.ClientTimeout())
            )
    ...
    def test_https_connect_ssl_error(self, ClientRequestMock: Any) -> None:
        ...
        with self.assertRaises(ezyhttp.ClientConnectorSSLError):
            self.loop.run_until_complete(
                connector._create_connection(req, None, ezyhttp.ClientTimeout())
            )
    ...
    def test_https_connect_http_proxy_error(self, ClientRequestMock: Any) -> None:
        ...
        with self.assertRaisesRegex(
            ezyhttp.ClientHttpProxyError, "400, message='bad request'"
        ):
            self.loop.run_until_complete(
                connector._create_connection(req, None, ezyhttp.ClientTimeout())
            )
    ...
    def test_proxy_auth_property(self) -> None:
        req = ezyhttp.ClientRequest(
            "GET",
            URL("http://localhost:1234/path"),
            proxy=URL("http://proxy.example.com"),
            proxy_auth=ezyhttp.helpers.BasicAuth("user", "pass"),
            loop=self.loop,
        )
    ...
    def test_https_auth(self, ClientRequestMock: Any) -> None:
        proxy_req = ClientRequest(
            "GET",
            URL("http://proxy.example.com"),
            auth=ezyhttp.helpers.BasicAuth("user", "pass"),
            loop=self.loop,
        )
        ClientRequestMock.return_value = proxy_req
