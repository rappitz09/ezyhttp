...
import ezyhttp
from ezyhttp import BaseConnector, hdrs, helpers, payload
from ezyhttp.client_reqrep import (
    ClientRequest,
    ClientResponse,
    Fingerprint,
    _gen_default_accept_encoding,
)
from ezyhttp.http import HttpVersion
...
def test_request_info(make_request: Any) -> None:
    ...
    assert req.request_info == ezyhttp.RequestInfo(url, "GET", req.headers, url)


def test_request_info_with_fragment(make_request: Any) -> None:
    req = make_request("get", "http://python.org/#urlfragment")
    assert req.request_info == ezyhttp.RequestInfo(
        URL("http://python.org/"),
        "GET",
        req.headers,
...
def test_basic_auth(make_request: Any) -> None:
    req = make_request(
        "get", "http://python.org", auth=ezyhttp.BasicAuth("nkim", "1234")
    )
...
def test_basic_auth_utf8(make_request: Any) -> None:
    req = make_request(
        "get", "http://python.org", auth=ezyhttp.BasicAuth("nkim", "секрет", "utf-8")
    )
    assert "AUTHORIZATION" in req.headers
...
def test_basic_auth_from_url_overridden(make_request: Any) -> None:
    req = make_request(
        "get", "http://garbage@python.org", auth=ezyhttp.BasicAuth("nkim", "1234")
    )
    assert "AUTHORIZATION" in req.headers
    assert "Basic bmtpbToxMjM0" == req.headers["AUTHORIZATION"]
    assert "python.org" == req.host
...
async def test_urlencoded_formdata_charset(loop: Any, conn: Any) -> None:
    req = ClientRequest(
        "post",
        URL("http://python.org"),
        data=ezyhttp.FormData({"hey": "you"}, charset="koi8-r"),
        loop=loop,
    )
    await req.send(conn)
...
async def test_formdata_boundary_from_headers(loop: Any, conn: Any) -> None:
    ...
    file_path = pathlib.Path(__file__).parent / "ezyhttp.png"
...
async def test_pass_falsy_data(loop: Any) -> None:
    with mock.patch("ezyhttp.client_reqrep.ClientRequest.update_body_from_data"):
...
async def test_content_encoding(loop: Any, conn: Any) -> None:
    ...
    with mock.patch("ezyhttp.client_reqrep.StreamWriter") as m_writer:
...
async def test_content_encoding_dont_set_headers_if_no_body(
    ...
    with mock.patch("ezyhttp.client_reqrep.http"):
        resp = await req.send(conn)
...
async def test_oserror_on_write_bytes(loop: Any, conn: Any) -> None:
    ...
    assert isinstance(exc, ezyhttp.ClientOSError)
...
def test_gen_default_accept_encoding(has_brotli: Any, expected: Any) -> None:
    with mock.patch("ezyhttp.client_reqrep.HAS_BROTLI", has_brotli):
