...
from ezyhttp import HttpVersion, web
from ezyhttp.client_exceptions import ServerDisconnectedError
from ezyhttp.http_parser import RawRequestMessage
from ezyhttp.streams import StreamReader
from ezyhttp.test_utils import make_mocked_request
from ezyhttp.web import HTTPRequestEntityTooLarge, HTTPUnsupportedMediaType
from ezyhttp.web_request import ETag
...
async def test_json_invalid_content_type(ezyhttp_client: Any) -> None:
    async def handler(request):
        body_text = await request.text()
        assert body_text == '{"some": "data"}'
...
@pytest.mark.xfail(
    ...
    reason="see https://github.com/rappitz09/ezyhttp/issues/4572",
)
async def test_handler_return_type(ezyhttp_client: Any) -> None:
