...
from ezyhttp.abc import AbstractAccessLogger, AbstractAsyncAccessLogger
from ezyhttp.typedefs import Handler
from ezyhttp.web_log import AccessLogger
from ezyhttp.web_response import Response
...
@pytest.mark.skipif(
    ...
    :py:meth:`ezyhttp.AccessLogger._format_t`):
...
def test_access_logger_atoms(
    ...
    request = mock.Mock(
        ...
        method="GET",
        path_qs="/path",
        version=ezyhttp.HttpVersion(1, 1),
        remote="127.0.0.2",
    )
...
async def test_exc_info_context(ezyhttp_raw_server: Any, ezyhttp_client: Any) -> None:
    exc_msg = None
    ...
    logger = mock.Mock()
    server = await ezyhttp_raw_server(handler, access_log_class=Logger, logger=logger)
...
async def test_async_logger(ezyhttp_raw_server: Any, ezyhttp_client: Any):
    msg = None
...
async def test_contextvars_logger(ezyhttp_server: Any, ezyhttp_client: Any):
    VAR = ContextVar("VAR")
    ...
    server = await ezyhttp_server(app, access_log_class=Logger)
...
def test_logger_does_nothing_when_disabled(caplog: pytest.LogCaptureFixture) -> None:
    ...
    mock_logger = logging.getLogger("test.ezyhttp.log")
