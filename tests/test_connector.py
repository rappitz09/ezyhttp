...
import ezyhttp
from ezyhttp import client, web
from ezyhttp.client import ClientRequest, ClientTimeout
from ezyhttp.client_reqrep import ConnectionKey
from ezyhttp.connector import Connection, TCPConnector, _DNSCacheTable
from ezyhttp.locks import EventResultOrError
from ezyhttp.test_utils import make_mocked_coro, unused_port
from ezyhttp.tracing import Trace
...
async def test_del_with_scheduled_cleanup(loop: Any) -> None:
    loop.set_debug(True)
    conn = ezyhttp.BaseConnector(keepalive_timeout=0.01)
...
async def test_get_expired_ssl(loop: Any) -> None:
    conn = ezyhttp.BaseConnector(enable_cleanup_closed=True)
    key = ConnectionKey("localhost", 80, True, None, None, None, None)
    assert conn._get(key) is None
...
async def test_release_acquired(key: Any) -> None:
    proto = create_mocked_conn()
    conn = ezyhttp.BaseConnector(limit=5)
...
async def test_release_waiter_no_limit(loop: Any, key: Any, key2: Any) -> None:
    # limit is 0
    conn = ezyhttp.BaseConnector(limit=0)
    w = mock.Mock()
...
async def test_release_waiter_release_first(loop: Any, key: Any, key2: Any) -> None:
    conn = ezyhttp.BaseConnector(limit=1)
    w1, w2 = mock.Mock(), mock.Mock()
    w1.done.return_value = False
    w2.done.return_value = False
...
async def test_release_waiter_per_host(loop: Any, key: Any, key2: Any) -> None:
    # no limit
    conn = ezyhttp.BaseConnector(limit=0, limit_per_host=2)
    w1, w2 = mock.Mock(), mock.Mock()
    w1.done.return_value = False
    w2.done.return_value = False
...
async def test_tcp_connector_certificate_error(loop: Any) -> None:
    ...
    with pytest.raises(ezyhttp.ClientConnectorCertificateError) as ctx:
    ...
    assert isinstance(ctx.value, ezyhttp.ClientSSLError)
...
async def test_tcp_connector_multiple_hosts_errors(loop: Any) -> None:
    ...
    req = ClientRequest(
        ...
        ssl=ezyhttp.Fingerprint(fingerprint),
        loop=loop,
    )
...
async def test_tcp_connector_resolve_host(loop: Any) -> None:
    conn = ezyhttp.TCPConnector(use_dns_cache=True)
...
async def test_tcp_connector_dns_cache_not_expired(
    ...
    with mock.patch("ezyhttp.connector.DefaultResolver") as m_resolver:
        conn = ezyhttp.TCPConnector(use_dns_cache=True, ttl_dns_cache=10)
...
async def test_tcp_connector_use_dns_cache_disabled(
    ...
    with mock.patch("ezyhttp.connector.DefaultResolver") as m_resolver:
        conn = ezyhttp.TCPConnector(use_dns_cache=False)
...
async def test_tcp_connector_dns_tracing(loop: Any, dns_response: Any) -> None:
    ...
    with mock.patch("ezyhttp.connector.DefaultResolver") as m_resolver:
        ...
        on_dns_resolvehost_start.assert_called_once_with(
            session,
            trace_config_ctx,
            ezyhttp.TraceDnsResolveHostStartParams("localhost"),
        )
        on_dns_resolvehost_end.assert_called_once_with(
            session, trace_config_ctx, ezyhttp.TraceDnsResolveHostEndParams("localhost")
        )
        on_dns_cache_miss.assert_called_once_with(
            session, trace_config_ctx, ezyhttp.TraceDnsCacheMissParams("localhost")
        )
        assert not on_dns_cache_hit.called

        await conn._resolve_host("localhost", 8080, traces=traces)
        on_dns_cache_hit.assert_called_once_with(
            session, trace_config_ctx, ezyhttp.TraceDnsCacheHitParams("localhost")
        )
        ...
        on_dns_resolvehost_end.assert_has_calls(
            [
                mock.call(
                    session,
                    trace_config_ctx,
                    ezyhttp.TraceDnsResolveHostEndParams("localhost"),
                ),
                mock.call(
                    session,
                    trace_config_ctx,
...
async def test_dns_error(loop: Any) -> None:
    connector = ezyhttp.TCPConnector()
    ...
    with pytest.raises(ezyhttp.ClientConnectorError):
...
async def test_connect_tracing(loop: Any) -> None:
    ...
    on_connection_create_start.assert_called_with(
        session, trace_config_ctx, ezyhttp.TraceConnectionCreateStartParams()
    )
    on_connection_create_end.assert_called_with(
        session, trace_config_ctx, ezyhttp.TraceConnectionCreateEndParams()
    )
...
async def test_ctor_cleanup() -> None:
    ...
    conn = ezyhttp.BaseConnector(keepalive_timeout=10, enable_cleanup_closed=True)
...
async def test_cleanup2(loop: Any) -> None:
    ...
    conn = ezyhttp.BaseConnector(keepalive_timeout=10)
...
async def test_cleanup_closed_disabled(loop: Any, mocker: Any) -> None:
    conn = ezyhttp.BaseConnector(enable_cleanup_closed=False)
...
async def test_invalid_ssl_param() -> None:
    with pytest.raises(TypeError):
        ezyhttp.TCPConnector(ssl=object())


async def test_tcp_connector_ctor_fingerprint_valid(loop: Any) -> None:
    valid = ezyhttp.Fingerprint(hashlib.sha256(b"foo").digest())
    conn = ezyhttp.TCPConnector(ssl=valid)
    assert conn._ssl is valid


async def test_insecure_fingerprint_md5(loop: Any) -> None:
    with pytest.raises(ValueError):
        ezyhttp.TCPConnector(ssl=ezyhttp.Fingerprint(hashlib.md5(b"foo").digest()))


async def test_insecure_fingerprint_sha1(loop: Any) -> None:
    with pytest.raises(ValueError):
        ezyhttp.TCPConnector(ssl=ezyhttp.Fingerprint(hashlib.sha1(b"foo").digest()))
...
async def test___get_ssl_context3(loop: Any) -> None:
    ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
    conn = ezyhttp.TCPConnector(ssl=ctx)
    req = mock.Mock()
    req.is_ssl.return_value = True
    req.ssl = None
    assert conn._get_ssl_context(req) is ctx
...
async def test___get_ssl_context5(loop: Any) -> None:
    ...
    req = mock.Mock()
    req.is_ssl.return_value = True
    req.ssl = ezyhttp.Fingerprint(hashlib.sha256(b"1").digest())
    assert conn._get_ssl_context(req) is conn._make_ssl_context(False)
...
async def test_connect_queued_operation_tracing(loop: Any, key: Any) -> None:
    ...
    async def f():
        ...
        on_connection_queued_start.assert_called_with(
            session, trace_config_ctx, ezyhttp.TraceConnectionQueuedStartParams()
        )
        on_connection_queued_end.assert_called_with(
            session, trace_config_ctx, ezyhttp.TraceConnectionQueuedEndParams()
        )
        connection2.release()
...
async def test_connect_reuseconn_tracing(loop: Any, key: Any) -> None:
    ...
    on_connection_reuseconn.assert_called_with(
        session, trace_config_ctx, ezyhttp.TraceConnectionReuseconnParams()
    )
    await conn.close()
...
async def test_connect_with_limit_and_limit_per_host(loop: Any, key: Any) -> None:
    ...
    conn = ezyhttp.BaseConnector(limit=1000, limit_per_host=1)
    conn._conns[key] = [(proto, loop.time())]
    conn._create_connection = mock.Mock()
    conn._create_connection.return_value = loop.create_future()
    conn._create_connection.return_value.set_result(proto)
...
async def test_connect_with_no_limit_and_limit_per_host(loop: Any, key: Any) -> None:
    ...
    conn = ezyhttp.BaseConnector(limit=0, limit_per_host=1)
    conn._conns[key] = [(proto, loop.time())]
    conn._create_connection = mock.Mock()
    conn._create_connection.return_value = loop.create_future()
    conn._create_connection.return_value.set_result(proto)
...
async def test_connect_with_no_limits(loop: Any, key: Any) -> None:
    ...
    conn = ezyhttp.BaseConnector(limit=0, limit_per_host=0)
    conn._conns[key] = [(proto, loop.time())]
    conn._create_connection = mock.Mock()
    conn._create_connection.return_value = loop.create_future()
    conn._create_connection.return_value.set_result(proto)
...
async def test_connect_with_limit_concurrent(loop: Any) -> None:
    ...
    conn = ezyhttp.BaseConnector(limit=max_connections)
...
async def test_limit_per_host_property(loop: Any) -> None:
    conn = ezyhttp.BaseConnector(limit_per_host=15)
...
async def test_force_close_and_explicit_keep_alive(loop: Any) -> None:
    with pytest.raises(ValueError):
        ezyhttp.BaseConnector(keepalive_timeout=30, force_close=True)

    conn = ezyhttp.BaseConnector(force_close=True, keepalive_timeout=None)
    assert conn

    conn = ezyhttp.BaseConnector(force_close=True)
...
async def test_tcp_connector(ezyhttp_client: Any, loop: Any) -> None:
...
async def test_unix_connector_not_found(loop: Any) -> None:
    connector = ezyhttp.UnixConnector("/" + uuid.uuid4().hex)
...
async def test_named_pipe_connector_wrong_loop(
    ...
    with pytest.raises(RuntimeError):
        ezyhttp.NamedPipeConnector(pipe_name)
...
async def test_named_pipe_connector_not_found(
    ...
    connector = ezyhttp.NamedPipeConnector(pipe_name)
...
async def test_tcp_connector_raise_connector_ssl_error(
    ezyhttp_server: Any, ssl_ctx: Any
) -> None:
    async def handler(request):
        return web.Response()
    ...
    srv = await ezyhttp_server(app, ssl=ssl_ctx)

    port = unused_port()
    conn = ezyhttp.TCPConnector(local_addr=("127.0.0.1", port))

    session = ezyhttp.ClientSession(connector=conn)
    url = srv.make_url("/")

    with pytest.raises(ezyhttp.ClientConnectorCertificateError) as ctx:
        await session.get(url)

    assert isinstance(ctx.value, ezyhttp.ClientConnectorCertificateError)
...
async def test_tcp_connector_do_not_raise_connector_ssl_error(
    ezyhttp_server: Any, ssl_ctx: Any, client_ssl_ctx: Any, host: str
) -> None:
    async def handler(request):
        return web.Response()
...
async def test_tcp_connector_uses_provided_local_addr(ezyhttp_server: Any) -> None:
    async def handler(request):
        return web.Response()

    app = web.Application()
    app.router.add_get("/", handler)
    srv = await ezyhttp_server(app)
...
async def test_unix_connector(unix_server: Any, unix_sockname: Any) -> None:
    ...
    connector = ezyhttp.UnixConnector(unix_sockname)
...
class TestDNSCacheTable:
    ...
    def test_expired_ttl(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("ezyhttp.connector.monotonic", lambda: 1)
        dns_cache_table.add("localhost", ["127.0.0.1"])
        monkeypatch.setattr("ezyhttp.connector.monotonic", lambda: 2)
        assert not dns_cache_table.expired("localhost")
        monkeypatch.setattr("ezyhttp.connector.monotonic", lambda: 3)
    ...
    def test_never_expire(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("ezyhttp.connector.monotonic", lambda: 1)
        dns_cache_table.add("localhost", ["127.0.0.1"])
        monkeypatch.setattr("ezyhttp.connector.monotonic", lambda: 10000000)
        assert not dns_cache_table.expired("localhost")
    ...
    def test_always_expire(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("ezyhttp.connector.monotonic", lambda: 1)
        dns_cache_table.add("localhost", ["127.0.0.1"])
        monkeypatch.setattr("ezyhttp.connector.monotonic", lambda: 1.00001)
        assert dns_cache_table.expired("localhost")
