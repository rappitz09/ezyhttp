...
from ezyhttp.web import HTTPMethodNotAllowed, HTTPNotFound, Response
from ezyhttp.web_urldispatcher import (
    PATH_SEP,
    AbstractResource,
    Domain,
    DynamicResource,
...
def fill_routes(router: Any):
    def go():
        ...
        resource = router.add_static("/static", pathlib.Path(ezyhttp.__file__).parent)
...
def test_add_static(router: Any) -> None:
    resource = router.add_static(
        "/st", pathlib.Path(ezyhttp.__file__).parent, name="static"
    )
...
def test_add_static_quoting(router: Any) -> None:
    resource = router.add_static(
        "/пре %2Fфикс", pathlib.Path(ezyhttp.__file__).parent, name="static"
    )
    assert router["static"] is resource
...
async def test_static_not_match(router: Any) -> None:
    router.add_static("/pre", pathlib.Path(ezyhttp.__file__).parent, name="name")
...
def test_static_repr(router: Any) -> None:
    router.add_static("/get", pathlib.Path(ezyhttp.__file__).parent, name="name")
    assert Matches(r"<StaticResource 'name' /get") == repr(router["name"])


def test_static_adds_slash(router: Any) -> None:
    route = router.add_static("/prefix", pathlib.Path(ezyhttp.__file__).parent)
...
def test_named_resources(router: Any) -> None:
    ...
    route3 = router.add_static(
        "/static", pathlib.Path(ezyhttp.__file__).parent, name="route3"
    )
...
def test_static_resource_get_info(router: Any) -> None:
    directory = pathlib.Path(ezyhttp.__file__).parent.resolve()
...
def test_static_route_user_home(router: Any) -> None:
    here = pathlib.Path(ezyhttp.__file__).parent
    try:
        static_dir = pathlib.Path("~") / here.relative_to(pathlib.Path.home())
    except ValueError:
        pytest.skip("ezyhttp folder is not placed in user's HOME")
...
def test_static_route_points_to_file(router: Any) -> None:
    here = pathlib.Path(ezyhttp.__file__).parent / "__init__.py"
    with pytest.raises(ValueError):
        router.add_static("/st", here)


async def test_404_for_static_resource(router: Any) -> None:
    resource = router.add_static("/st", pathlib.Path(ezyhttp.__file__).parent)
...
def test_static_resource_canonical() -> None:
    prefix = "/prefix"
    directory = str(pathlib.Path(ezyhttp.__file__).parent)
    canonical = prefix
