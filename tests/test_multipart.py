...
import ezyhttp
from ezyhttp import payload
from ezyhttp.hdrs import (
    CONTENT_DISPOSITION,
    CONTENT_ENCODING,
    CONTENT_TRANSFER_ENCODING,
    CONTENT_TYPE,
)
from ezyhttp.helpers import parse_mimetype
from ezyhttp.multipart import MultipartResponseWrapper
from ezyhttp.streams import StreamReader
...
@pytest.fixture
def writer():
    return ezyhttp.MultipartWriter(boundary=":")
...
class TestPartReader:
    async def test_next(self, newline: Any) -> None:
        ...
        with Stream(data) as stream:
            obj = ezyhttp.BodyPartReader(BOUNDARY, {}, stream, _newline=newline)
    ...
    async def test_read_chunk_at_eof(self) -> None:
        with Stream(b"--:") as stream:
            obj = ezyhttp.BodyPartReader(BOUNDARY, {}, stream)
            obj._at_eof = True
    ...
    async def test_read_text_while_closed(self) -> None:
        with Stream(b"") as stream:
            obj = ezyhttp.BodyPartReader(BOUNDARY, {CONTENT_TYPE: "text/plain"}, stream)
            obj._at_eof = True
            result = await obj.text()
        assert "" == result
...
class TestMultipartReader:
    def test_from_response(self, newline: Any) -> None:
        with Stream(b"--:%s\r\nhello%s--:--" % (newline, newline)) as stream:
            ...
            res = ezyhttp.MultipartReader.from_response(resp)
        assert isinstance(res, MultipartResponseWrapper)
        assert isinstance(res.stream, ezyhttp.MultipartReader)
    ...
    def test_bad_boundary(self) -> None:
        with Stream(b"") as stream:
            ...
            with pytest.raises(ValueError):
                ezyhttp.MultipartReader.from_response(resp)

    def test_dispatch(self, newline: Any) -> None:
        with Stream(b"--:%s\r\necho%s--:--" % (newline, newline)) as stream:
            reader = ezyhttp.MultipartReader(
                {CONTENT_TYPE: 'multipart/related;boundary=":"'},
                stream,
            )
    ...
    def test_dispatch_custom_multipart_reader(self, newline: Any) -> None:
        class CustomReader(ezyhttp.MultipartReader):
            pass
    ...
    async def test_second_next_releases_previous_object(self, newline: Any) -> None:
        with Stream(
            ...
            assert isinstance(first, ezyhttp.BodyPartReader)
...
async def test_writer_write(buf: Any, stream: Any, writer: Any) -> None:
    ...
    sub_multipart = ezyhttp.MultipartWriter(boundary="::")
...
async def test_writer_write_no_close_boundary(buf: Any, stream: Any) -> None:
    writer = ezyhttp.MultipartWriter(boundary=":")
    writer.append("foo-bar-baz")
    writer.append_json({"test": "passed"})
    writer.append_form({"test": "passed"})
    writer.append_form([("one", 1), ("two", 2)])
...
class TestMultipartWriter:
    ...
    def test_unquoted_boundary(self) -> None:
        writer = ezyhttp.MultipartWriter(boundary="abc123")
        expected = {CONTENT_TYPE: "multipart/mixed; boundary=abc123"}
        assert expected == writer.headers

    def test_quoted_boundary(self) -> None:
        writer = ezyhttp.MultipartWriter(boundary=R"\"")
        expected = {CONTENT_TYPE: R'multipart/mixed; boundary="\\\""'}
        assert expected == writer.headers

    def test_bad_boundary(self) -> None:
        with pytest.raises(ValueError):
            ezyhttp.MultipartWriter(boundary="тест")
        with pytest.raises(ValueError):
            ezyhttp.MultipartWriter(boundary="test\n")
        with pytest.raises(ValueError):
            ezyhttp.MultipartWriter(boundary="X" * 71)
    ...
    def test_append_multipart(self, writer: Any) -> None:
        subwriter = ezyhttp.MultipartWriter(boundary=":")
    ...
    def test_with(self) -> None:
        with ezyhttp.MultipartWriter(boundary=":") as writer:
            writer.append("foo")
            writer.append(b"bar")
    ...
    async def test_preserve_content_disposition_header(
        ...
        # https://github.com/rappitz09/ezyhttp/pull/3475#issuecomment-451072381
        with pathlib.Path(__file__).open("rb") as fobj:
            with ezyhttp.MultipartWriter("form-data", boundary=":") as writer:
                part = writer.append(
                    fobj,
                    headers={
...
async def test_async_for_reader() -> None:
    data = [{"test": "passed"}, 42, b"plain text", b"ezyhttp\n", b"no epilogue"]
    with Stream(
        b"\r\n".join(
            [
                b"--:",
                ...
                data[2],
                b"--::",
                b'Content-Disposition: attachment; filename="ezyhttp"',
                b"Content-Type: text/plain",
                b"Content-Length: 28",
        ...
        async def check(reader):
            async for part in reader:
                if isinstance(part, ezyhttp.BodyPartReader):
...
async def test_async_for_bodypart() -> None:
    with Stream(b"foobarbaz\r\n--:--") as stream:
        part = ezyhttp.BodyPartReader(boundary=b"--:", headers={}, content=stream)
