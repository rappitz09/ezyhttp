...
import ezyhttp
from ezyhttp import http
from ezyhttp.client_reqrep import ClientResponse, RequestInfo
...
async def test_json_invalid_content_type(loop: Any, session: Any) -> None:
    ...
    with pytest.raises(ezyhttp.ContentTypeError) as info:
        await response.json()
...
def test_raise_for_status_4xx() -> None:
    ...
    with pytest.raises(ezyhttp.ClientResponseError) as cm:
