...
from ezyhttp import ClientConnectorError, ClientSession, WSCloseCode, web
from ezyhttp.test_utils import make_mocked_coro
from ezyhttp.web_runner import BaseRunner
...
def test_run_app_nondefault_host_port(
    patched_loop: Any, ezyhttp_unused_port: Any
) -> None:
...
class TestShutdown:
    ...
    def test_shutdown_wait_for_task(
        self, ezyhttp_unused_port: Callable[[], int]
    ) -> None:
        port = ezyhttp_unused_port()
        finished = False
