...
import ezyhttp
from ezyhttp import WSServerHandshakeError, web
from ezyhttp.http import WSCloseCode, WSMsgType


async def test_websocket_can_prepare(loop: Any, ezyhttp_client: Any) -> None:
...
async def test_websocket_json(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        if not ws.can_prepare(request):
...
async def test_websocket_json_invalid_message(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        try:
...
async def test_websocket_send_json(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_websocket_receive_json(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_send_recv_text(loop: Any, ezyhttp_client: Any) -> None:
...
async def test_send_recv_bytes(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def test_send_recv_json(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def test_close_timeout(loop: Any, ezyhttp_client: Any) -> None:
...
async def test_concurrent_close(loop: Any, ezyhttp_client: Any) -> None:
    srv_ws = None
...
async def test_auto_pong_with_closing_by_peer(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def test_ping(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def ezyhttp_client_ping(loop: Any, ezyhttp_client: Any):
    closed = loop.create_future()
...
async def test_pong(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def test_change_status(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def test_handle_protocol(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def test_server_close_handshake(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def ezyhttp_client_close_handshake(loop: Any, ezyhttp_client: Any):
    closed = loop.create_future()
...
async def test_receive_timeout(loop: Any, ezyhttp_client: Any) -> None:
    raised = False
...
async def test_custom_receive_timeout(loop: Any, ezyhttp_client: Any) -> None:
    raised = False
...
async def test_heartbeat(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_heartbeat_no_pong(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse(heartbeat=0.05)
        await ws.prepare(request)
...
async def test_server_ws_async_for(loop: Any, ezyhttp_server: Any) -> None:
    closed = loop.create_future()
    ...
    async with ezyhttp.ClientSession() as sm:
...
async def test_closed_async_for(loop: Any, ezyhttp_client: Any) -> None:
    closed = loop.create_future()
...
async def test_websocket_disable_keepalive(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        if not ws.can_prepare(request):
...
async def test_receive_str_nonstring(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        if not ws.can_prepare(request):
            return web.HTTPUpgradeRequired()
...
async def test_receive_bytes_nonbytes(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        if not ws.can_prepare(request):
            return web.HTTPUpgradeRequired()
...
async def test_bug3380(loop: Any, ezyhttp_client: Any) -> None:
    async def handle_null(request):
        return ezyhttp.web.json_response({"err": None})
    ...
    resp = await client.get("/api/null", timeout=ezyhttp.ClientTimeout(total=1))
    assert (await resp.json()) == {"err": None}
    resp.close()
