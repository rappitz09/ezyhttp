...
import ezyhttp
from ezyhttp import http_exceptions, streams
from ezyhttp.http_parser import (
    NO_EXTENSIONS,
    DeflateBuffer,
    HttpPayloadParser,
    HttpRequestParserPy,
...
try:
    from ezyhttp.http_parser import HttpRequestParserC, HttpResponseParserC
...
def _gen_ids(parsers: List[Any]) -> List[str]:
    return [
        "py-parser" if parser.__module__ == "ezyhttp.http_parser" else "c-parser"
...
def test_c_parser_loaded():
    assert "HttpRequestParserC" in dir(ezyhttp.http_parser)
    assert "HttpResponseParserC" in dir(ezyhttp.http_parser)
    assert "RawRequestMessageC" in dir(ezyhttp.http_parser)
    assert "RawResponseMessageC" in dir(ezyhttp.http_parser)
...
class TestParsePayload:
    ...
    async def test_parse_chunked_payload_split_end(self, protocol: Any) -> None:
        out = ezyhttp.StreamReader(protocol, 2**16, loop=None)
        p = HttpPayloadParser(out, chunked=True)
        p.feed_data(b"4\r\nasdf\r\n0\r\n")
        p.feed_data(b"\r\n")
...
class TestDeflateBuffer:
    async def test_feed_data(self, stream: Any) -> None:
        buf = ezyhttp.FlowControlDataQueue(
            stream, 2**16, loop=asyncio.get_event_loop()
        )
