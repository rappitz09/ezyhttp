...
def test_appkey_repr_concrete() -> None:
    ...
    assert repr(key2) in (
        # pytest-xdist:
        "<AppKey(__channelexec__.key, type=ezyhttp.web_request.Request)>",
        "<AppKey(__main__.key, type=ezyhttp.web_request.Request)>",
    )
...
async def test_subapp_chained_config_dict_visibility(ezyhttp_client: Any) -> None:
    ...
    client = await ezyhttp_client(root)
...
async def test_subapp_chained_config_dict_overriding(ezyhttp_client: Any) -> None:
    key = web.AppKey("key", str)
...
async def test_subapp_on_startup(ezyhttp_client: Any) -> None:
    subapp = web.Application()
