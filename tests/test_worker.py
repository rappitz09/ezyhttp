...
if TYPE_CHECKING:
    from ezyhttp import worker as base_worker
else:
    base_worker = pytest.importorskip("ezyhttp.worker")
...
def test_init_process(worker: base_worker.GunicornWebWorker) -> None:
    with mock.patch("ezyhttp.worker.asyncio") as m_asyncio:
        try:
            worker.init_process()
        except TypeError:
            pass
...
def test_handle_abort(worker: base_worker.GunicornWebWorker) -> None:
    with mock.patch("ezyhttp.worker.sys") as m_sys:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            AsyncioWorker.DEFAULT_ezyhttp_LOG_FORMAT,
        ),
    ],
)
...
async def test__run_ok_parent_changed(
    ...
    ezyhttp_unused_port: Callable[[], int],
) -> None:
    worker.ppid = 0
    worker.alive = True
    sock = socket.socket()
    addr = ("localhost", ezyhttp_unused_port())
    sock.bind(addr)
    worker.sockets = [sock]
    worker.log = mock.Mock()
    worker.loop = loop
