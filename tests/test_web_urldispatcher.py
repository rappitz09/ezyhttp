...
from ezyhttp.pytest_plugin import ezyhttpClient
from ezyhttp.web_urldispatcher import Resource, SystemRoute
...
async def test_follow_symlink(
    tmp_path: pathlib.Path, ezyhttp_client: ezyhttpClient
) -> None:
...
async def test_url_escaping(
    ezyhttp_client: ezyhttpClient, registered_path: str, request_url: str
) -> None:
...
async def test_partially_applied_handler(ezyhttp_client: ezyhttpClient) -> None:
    app = web.Application()
...
async def test_static_head(
    ...
    # done with ezyhttp_client because the buffering can consume the content.
...
async def test_allow_head(ezyhttp_client: ezyhttpClient) -> None:
...
async def test_add_view(ezyhttp_client: ezyhttpClient) -> None:
    app = web.Application()
...
async def test_decorate_view(ezyhttp_client: ezyhttpClient) -> None:
...
async def test_web_view(ezyhttp_client: ezyhttpClient) -> None:
    app = web.Application()
...
async def test_static_absolute_url(
    ezyhttp_client: ezyhttpClient, tmp_path: pathlib.Path
) -> None:
...
@pytest.mark.xfail(
    ...
    reason="Regression in v3.7: https://github.com/rappitz09/ezyhttp/issues/5621",
)
@pytest.mark.parametrize(
...
async def test_order_is_preserved(ezyhttp_client: ezyhttpClient) -> None:
...
async def test_url_with_many_slashes(ezyhttp_client: ezyhttpClient) -> None:
    app = web.Application()
