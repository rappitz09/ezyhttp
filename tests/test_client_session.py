...
import ezyhttp
from ezyhttp import client, hdrs, web
from ezyhttp.client import ClientSession
from ezyhttp.client_reqrep import ClientRequest
from ezyhttp.connector import BaseConnector, TCPConnector
...
def test_http_GET(session: Any, params: Any) -> None:
    with mock.patch(
        "ezyhttp.client.ClientSession._request", new_callable=mock.MagicMock
    ) as patched:
...
async def test_reraise_os_error(create_session: Any, create_mocked_conn: Any) -> None:
    ...
    with pytest.raises(ezyhttp.ClientOSError) as ctx:
...
async def test_cookie_jar_usage(loop: Any, ezyhttp_client: Any) -> None:
    req_url = None
...
async def test_session_default_version(loop: Any) -> None:
    ...
    assert session.version == ezyhttp.HttpVersion11
    await session.close()
...
async def test_request_tracing(loop: Any, ezyhttp_client: Any) -> None:
    async def handler(request):
    ...
    with io.BytesIO() as gathered_req_body, io.BytesIO() as gathered_res_body:
        ...
        async with session.post(
            ...
            on_request_start.assert_called_once_with(
                session.session,
                trace_config_ctx,
                ezyhttp.TraceRequestStartParams(
...
async def test_request_tracing_url_params(loop: Any, ezyhttp_client: Any) -> None:
    ...
    session = await ezyhttp_client(app, trace_configs=[trace_config])
    ...
    # Exception
    with mock.patch("ezyhttp.client.TCPConnector.connect") as connect_patched:
...
async def test_request_tracing_exception() -> None:
    ...
    with mock.patch("ezyhttp.client.TCPConnector.connect") as connect_patched:
        ...
        session = ezyhttp.ClientSession(trace_configs=[trace_config])
        ...
        on_request_exception.assert_called_once_with(
            session,
            mock.ANY,
            ezyhttp.TraceRequestExceptionParams(
