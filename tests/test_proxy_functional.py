...
from ezyhttp.client_exceptions import ClientConnectionError
...
async def web_server_endpoint_url(
    ...
    server = await ezyhttp_server(app, **server_kwargs)
...
async def test_secure_https_proxy_absolute_path(
    ...
    sess = ezyhttp.ClientSession(connector=conn)
...
async def test_https_proxy_unsupported_tls_in_tls(
    ...
    expected_warning_text = (
        ...
        r"\* https://docs\.ezyhttp\.org/en/stable/client_advanced\.html#proxy-support\n"
        r"\* https://github\.com/rappitz09/ezyhttp/discussions/6044\n$"
    )
    type_err = (
...
@pytest.fixture
def proxy_test_server(ezyhttp_raw_server: Any, loop: Any, monkeypatch: Any):
    ...
    async def proxy_server():
        ...
        server = await ezyhttp_raw_server(proxy_handler)
...
def get_request(loop: Any):
    async def _request(method="GET", *, url, trust_env=False, **kwargs):
        connector = ezyhttp.TCPConnector(ssl=False)
        client = ezyhttp.ClientSession(connector=connector, trust_env=trust_env)
        try:
...
async def test_proxy_http_absolute_path(
    ...
    url = "http://ezyhttp.io/path?query=yes"
...
async def test_proxy_http_auth_utf8(proxy_test_server: Any, get_request: Any) -> None:
    url = "http://ezyhttp.io/path"
    auth = ezyhttp.BasicAuth("юзер", "пасс", "utf-8")
    proxy = await proxy_test_server()
...
async def test_proxy_http_acquired_cleanup_force(
    ...
    conn = ezyhttp.TCPConnector(force_close=True)
    sess = ezyhttp.ClientSession(connector=conn)
    proxy = await proxy_test_server()
...
async def test_proxy_http_multi_conn_limit(proxy_test_server: Any, loop: Any) -> None:
    ...
    conn = ezyhttp.TCPConnector(limit=limit)
    sess = ezyhttp.ClientSession(connector=conn)
    proxy = await proxy_test_server()
...
async def xtest_proxy_https_connect(proxy_test_server: Any, get_request: Any) -> None:
    proxy = await proxy_test_server()
    url = "https://www.google.com.ua/search?q=ezyhttp proxy"
    ...
    assert proxy.request.path_qs == "/search?q=ezyhttp+proxy"
...
async def xtest_proxy_https_connect_with_port(
    ...
    proxy = await proxy_test_server()
    url = "https://secure.ezyhttp.io:2242/path"

    await get_request(url=url, proxy=proxy.url)

    connect = proxy.requests_list[0]
    assert connect.method == "CONNECT"
    assert connect.path == "secure.ezyhttp.io:2242"
    assert connect.host == "secure.ezyhttp.io:2242"

    assert proxy.request.host == "secure.ezyhttp.io:2242"
    assert proxy.request.path_qs == "/path"


@pytest.mark.xfail
async def xtest_proxy_https_send_body(proxy_test_server: Any, loop: Any) -> None:
    sess = ezyhttp.ClientSession()
    proxy = await proxy_test_server()
...
async def test_proxy_https_connection_error(get_request: Any) -> None:
    url = "https://secure.ezyhttp.io/path"
    proxy_url = "http://localhost:2242/"
...
async def test_proxy_https_bad_response(
    ...
    with pytest.raises(ezyhttp.ClientHttpProxyError):
        await get_request(url=url, proxy=proxy.url)

    assert len(proxy.requests_list) == 1
    assert proxy.request.method == "CONNECT"
    # The following check fails on MacOS
    # assert proxy.request.path == 'secure.ezyhttp.io:443'
