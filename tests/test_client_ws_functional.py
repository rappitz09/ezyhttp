...
import ezyhttp
from ezyhttp import hdrs, web
from ezyhttp.client_ws import ClientWSTimeout
from ezyhttp.http import WSCloseCode
...
async def test_send_recv_text(ezyhttp_client: Any) -> None:
...
async def test_send_recv_bytes_bad_type(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_send_recv_bytes(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_send_recv_text_bad_type(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_send_recv_json(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_ping_pong(ezyhttp_client: Any) -> None:
    ...
    assert msg.type == ezyhttp.WSMsgType.BINARY
    assert msg.data == b"ask/answer"

    msg = await resp.receive()
    assert msg.type == ezyhttp.WSMsgType.CLOSE

    await resp.close()
    await closed


async def test_ping_pong_manual(ezyhttp_client: Any) -> None:
    loop = asyncio.get_event_loop()
    closed = loop.create_future()
    ...
    msg = await resp.receive()
    assert msg.type == ezyhttp.WSMsgType.PONG

    msg = await resp.receive()
    assert msg.type == ezyhttp.WSMsgType.PING
    await resp.pong()
...
async def test_close(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
    ...
    msg = await resp.receive()
    assert msg.type == ezyhttp.WSMsgType.CLOSED


async def test_concurrent_task_close(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        await ws.receive()
...
async def test_concurrent_close(ezyhttp_client: Any) -> None:
    client_ws = None
    ...
    msg = await ws.receive()
    assert msg.type == ezyhttp.WSMsgType.CLOSING
...
async def test_close_from_server(ezyhttp_client: Any) -> None:
    loop = asyncio.get_event_loop()
    closed = loop.create_future()
...
async def test_close_manual(ezyhttp_client: Any) -> None:
    loop = asyncio.get_event_loop()
    closed = loop.create_future()
...
async def test_close_timeout_sock_close_read(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        await ws.receive_bytes()
    ...
    msg = await resp.receive()
    assert msg.data == "test"
    assert msg.type == ezyhttp.WSMsgType.TEXT
...
async def test_close_timeout_deprecated(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        await ws.receive_bytes()
...
async def test_close_cancel(ezyhttp_client: Any) -> None:
    loop = asyncio.get_event_loop()
...
async def test_override_default_headers(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_additional_headers(ezyhttp_client: Any) -> None:
    async def handler(request):
...
async def test_recv_protocol_error(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
    ...
    msg = await resp.receive()
    assert msg.type == ezyhttp.WSMsgType.ERROR
    assert type(msg.data) is ezyhttp.WebSocketError
    assert msg.data.code == ezyhttp.WSCloseCode.PROTOCOL_ERROR
    assert str(msg.data) == "Received frame with non-zero reserved bits"
    assert msg.extra is None
    await resp.close()


async def test_recv_timeout(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_receive_timeout_sock_read(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        await ws.receive()
...
async def test_receive_timeout_deprecation(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        await ws.receive()
...
async def test_custom_receive_timeout(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        await ws.receive()
...
async def test_heartbeat(ezyhttp_client: Any) -> None:
    ping_received = False
    ...
    async def handler(request):
        ...
        await ws.prepare(request)
        msg = await ws.receive()
        if msg.type == ezyhttp.WSMsgType.PING:
            ping_received = True
        await ws.close()
        return ws
...
async def test_heartbeat_no_pong(ezyhttp_client: Any) -> None:
    ping_received = False
...
async def test_send_recv_compress(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_send_recv_compress_wbits(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_send_recv_compress_wbit_error(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_ws_client_async_for(ezyhttp_client: Any) -> None:
    items = ["q1", "q2", "q3"]
...
async def test_ws_async_with(ezyhttp_server: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        msg = await ws.receive()
...
async def test_ws_async_with_send(ezyhttp_server: Any) -> None:
...
async def test_ws_async_with_shortcut(ezyhttp_server: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
        msg = await ws.receive()
...
async def test_closed_async_for(ezyhttp_client: Any) -> None:
    loop = asyncio.get_event_loop()
    closed = loop.create_future()
    ...
    assert messages[0].type == ezyhttp.WSMsgType.BINARY
...
async def test_peer_connection_lost(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
...
async def test_peer_connection_lost_iter(ezyhttp_client: Any) -> None:
    async def handler(request):
        ws = web.WebSocketResponse()
        await ws.prepare(request)
