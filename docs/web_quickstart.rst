...
:ref:`request handler <ezyhttp-web-handler>`.
...
*ezyhttp* server documentation uses both ways in code snippets to
emphasize their equality, switching from one style to another is very
trivial.

.. note::
   You can get a powerful ezyhttp template by running one command.
   To do this, simply use our `boilerplate for quick start with ezyhttp
   <https://create-aio-app.readthedocs.io/pages/ezyhttp_quick_start.html>`_.


.. seealso::

   :ref:`ezyhttp-web-graceful-shutdown` section explains what :func:`run_app`
   does and how to implement complex server initialization/finalization
   from scratch.

   :ref:`ezyhttp-web-app-runners` for more handling more complex cases
...
----------------------------
:mod:`ezyhttp.web` implements a basic CLI for quickly serving an
:class:`Application` in *development* over TCP/IP:

.. code-block:: shell

    $ python -m ezyhttp.web -H localhost -P 8080 package.module:init_func
...
:attr:`ezyhttp.web.BaseRequest.method` property.
...
.. seealso::
   ...
   :ref:`ezyhttp-web-peer-disconnection` section explains how handlers
   behave when a client connection drops and ways to optimize handling
   of this.

.. _ezyhttp-web-resource-and-route:
...
:ref:`request handler <ezyhttp-web-handler>` to access the matched value for
...
in a :ref:`request handler <ezyhttp-web-handler>`)::
...
resources <ezyhttp-web-variable-handler>`::
...
As discussed above, :ref:`handlers <ezyhttp-web-handler>` can be first-class
coroutines::
...
Since :mod:`ezyhttp.web` does not dictate any implementation details,
...
.. _ezyhttp-web-class-based-views:

Class Based Views
^^^^^^^^^^^^^^^^^

:mod:`ezyhttp.web` has support for *class based views*.
...
.. _ezyhttp-web-alternative-routes-definition:
...
The snippet calls :meth:`~ezyhttp.web.UrlDispatcher.add_routes` to
register a list of *route definitions* (:class:`ezyhttp.web.RouteDef`
instances) created by :func:`ezyhttp.web.get` or
:func:`ezyhttp.web.post` functions.

.. seealso:: :ref:`ezyhttp-web-route-def` reference.
...
The example creates a :class:`ezyhttp.web.RouteTableDef` container first.

The container is a list-like object with additional decorators
:meth:`ezyhttp.web.RouteTableDef.get`,
:meth:`ezyhttp.web.RouteTableDef.post` etc. for registering new
routes.

After filling the container
:meth:`~ezyhttp.web.UrlDispatcher.add_routes` is used for adding
registered *route definitions* into application's router.

.. seealso:: :ref:`ezyhttp-web-route-table-def` reference.
...
It is a common case to return JSON data in response, :mod:`ezyhttp.web`
provides a shortcut for returning JSON -- :func:`ezyhttp.web.json_response`::

   async def handler(request):
       data = {'some': 'data'}
       return web.json_response(data)

The shortcut method returns :class:`ezyhttp.web.Response` instance
...
:mod:`ezyhttp.web` has no built-in concept of a *session*, however, there is a
third-party library, :mod:`ezyhttp_session`, that adds *session* support::
    ...
    from ezyhttp import web
    from ezyhttp_session import setup, get_session, session_middleware
    from ezyhttp_session.cookie_storage import EncryptedCookieStorage
...
:attr:`ezyhttp.web.BaseRequest.query` for getting form data.

To access form data with ``"POST"`` method use
:meth:`ezyhttp.web.BaseRequest.post` or :meth:`ezyhttp.web.BaseRequest.multipart`.

:meth:`ezyhttp.web.BaseRequest.post` accepts both
...
For efficiency use :meth:`ezyhttp.web.BaseRequest.multipart`, It is especially effective
for uploading large files (:ref:`ezyhttp-web-file-upload`).
...
.. _ezyhttp-web-file-upload:

File Uploads
------------

:mod:`ezyhttp.web` has built-in support for handling files uploaded from the
browser.
...
Then, in the :ref:`request handler <ezyhttp-web-handler>` you can access the
...
that :meth:`ezyhttp.web.BaseRequest.post` reads the whole payload in memory,
resulting in possible
:abbr:`OOM (Out Of Memory)` errors. To avoid this, for multipart uploads, you
should use :meth:`ezyhttp.web.BaseRequest.multipart` which returns a :ref:`multipart reader
<ezyhttp-multipart>`::
...
.. _ezyhttp-web-websockets:

WebSockets
----------

:mod:`ezyhttp.web` supports *WebSockets* out-of-the-box.

To setup a *WebSocket*, create a :class:`WebSocketResponse` in a
:ref:`request handler <ezyhttp-web-handler>` and then use it to communicate
with the peer::
    ...
    async def websocket_handler(request):
        ...
        async for msg in ws:
            ...
            if msg.type == ezyhttp.WSMsgType.TEXT:
                if msg.data == 'close':
                    await ws.close()
                else:
                    await ws.send_str(msg.data + '/answer')
            elif msg.type == ezyhttp.WSMsgType.ERROR:
...
Example with login validation::
    ...
    @ezyhttp_jinja2.template('login.html')
