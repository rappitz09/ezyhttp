...
.. _ezyhttp-client-advanced:

Advanced Client Usage
=====================

.. _ezyhttp-client-session:
...
You also can set default headers for all session requests::
    ...
    async with ezyhttp.ClientSession(headers=headers) as session:
...
:class:`~ezyhttp.ClientSession` may be used for sharing cookies
...
.. note::
   ...
   redirection requests please use :ref:`ezyhttp.ClientSession
   <ezyhttp-client-session>` object.
...
.. _ezyhttp-client-cookie-safety:

Cookie Safety
^^^^^^^^^^^^^

By default :class:`~ezyhttp.ClientSession` uses strict version of
:class:`ezyhttp.CookieJar`. :rfc:`2109` explicitly forbids cookie
...
:class:`ezyhttp.CookieJar` constructor::


   jar = ezyhttp.CookieJar(unsafe=True)
   session = ezyhttp.ClientSession(cookie_jar=jar)


.. _ezyhttp-client-cookie-quoting-routine:

Cookie Quoting Routine
^^^^^^^^^^^^^^^^^^^^^^

The client uses the :class:`~ezyhttp.SimpleCookie` quoting routines
...
:class:`~ezyhttp.CookieJar` constructor::
   ...
   jar = ezyhttp.CookieJar(quote_cookie=False)
   session = ezyhttp.ClientSession(cookie_jar=jar)


.. _ezyhttp-client-dummy-cookie-jar:

Dummy Cookie Jar
^^^^^^^^^^^^^^^^

Sometimes cookie processing is not desirable. For this purpose it's
possible to pass :class:`ezyhttp.DummyCookieJar` instance into client
session::
...
ezyhttp, call the request function with the used compression algorithm
...
.. seealso:: :ref:`ezyhttp-client-tracing-reference` section for
...
*connector* to :class:`~ezyhttp.ClientSession` and family. For example::
...
.. seealso:: :ref:`ezyhttp-client-reference-connectors` section for
...
parameter to *connector*::
    ...
    conn = ezyhttp.TCPConnector(limit=30)
...
If you explicitly want not to have limits, pass `0`. For example::
    ...
    conn = ezyhttp.TCPConnector(limit=0)
...
parameter to *connector*::
    ...
    conn = ezyhttp.TCPConnector(limit_per_host=30)
...
By default :class:`~ezyhttp.TCPConnector` comes with the DNS cache
...
as can be seen in the following example::
    ...
    conn = ezyhttp.TCPConnector(ttl_dns_cache=300)
...
:term:`aiodns` is required::
    ...
    from ezyhttp.resolver import AsyncResolver

    resolver = AsyncResolver(nameservers=["8.8.8.8", "8.8.4.4"])
    conn = ezyhttp.TCPConnector(resolver=resolver)
...
:class:`~ezyhttp.UnixConnector`::

  conn = ezyhttp.UnixConnector(path='/path/to/socket')
  session = ezyhttp.ClientSession(connector=conn)
...
:class:`~ezyhttp.NamedPipeConnector`::

  conn = ezyhttp.NamedPipeConnector(path=r'\\.\pipe\<name-of-pipe>')
  session = ezyhttp.ClientSession(connector=conn)
...
By default *ezyhttp* uses strict checks for HTTPS protocol. Certification
...
:class:`ezyhttp.ClientConnectorSSLError`::

  try:
      await session.get('https://expired.badssl.com/')
  except ezyhttp.ClientConnectorSSLError as e:
      assert isinstance(e, ssl.SSLError)

:class:`ezyhttp.ClientConnectorCertificateError`::

  try:
      await session.get('https://wrong.host.badssl.com/')
  except ezyhttp.ClientConnectorCertificateError as e:
      assert isinstance(e, ssl.CertificateError)

If you need to skip both ssl related errors

:class:`ezyhttp.ClientSSLError`::

  try:
      await session.get('https://expired.badssl.com/')
  except ezyhttp.ClientSSLError as e:
      assert isinstance(e, ssl.SSLError)
...
You may also verify certificates via *SHA256* fingerprint::
  ...
  try:
      r = await session.get('https://www.python.org',
                            ssl=ezyhttp.Fingerprint(bad_fp))
  except ezyhttp.FingerprintMismatch as e:
      exc = e
...
.. _ezyhttp-client-proxy-support:

Proxy support
-------------

ezyhttp supports plain HTTP proxies and HTTP proxies that can be
upgraded to HTTPS via the HTTP CONNECT method. ezyhttp has a limited
...
It also supports proxy authorization::
   ...
   async with ezyhttp.ClientSession() as session:
       proxy_auth = ezyhttp.BasicAuth('user', 'pass')
       async with session.get("http://python.org",
...
``trust_env=True`` into :class:`ezyhttp.ClientSession`
constructor.::

   async with ezyhttp.ClientSession(trust_env=True) as session:
       async with session.get("http://python.org") as resp:
           print(resp.status)

.. note::
    ezyhttp uses :func:`urllib.request.getproxies`
...
:class:`ezyhttp.ClientSession` for more details).
...
.. attention::
   ...
   ezyhttp v3.8 and higher is ready for this to happen and has code in
   ...
   TLS-in-TLS, please don't file bugs with ezyhttp but rather try to
   ...
   *really* need this to work, there's a patch that may help you make
   ...
   https://github.com/rappitz09/ezyhttp/discussions/6044#discussioncomment-1432443.
...
.. _ezyhttp-persistent-session:

Persistent session
------------------

Even though creating a session on demand seems like a tempting idea, we
advise against it. :class:`ezyhttp.ClientSession` maintains a
...
**persistent session**, you can use either :ref:`ezyhttp-web-signals` or
:ref:`ezyhttp-web-cleanup-ctx`. If possible we advise using :ref:`ezyhttp-web-cleanup-ctx`,
as it results in more compact code::

    app.cleanup_ctx.append(persistent_session)
    persistent_session = ezyhttp.web.AppKey("persistent_session", ezyhttp.ClientSession)

    async def persistent_session(app):
       app[persistent_session] = session = ezyhttp.ClientSession()
       yield
       await session.close()
...
requirements. It benefits from having a single location where :class:`ezyhttp.ClientSession`
instances are created and where artifacts such as :class:`ezyhttp.BaseConnector`
...
In the end all you have to do is to close all sessions after the `yield` statement::
    ...
    async def multiple_sessions(app):
       app[persistent_session_1] = session_1 = ezyhttp.ClientSession()
       app[persistent_session_2] = session_2 = ezyhttp.ClientSession()
       app[persistent_session_3] = session_3 = ezyhttp.ClientSession()
...
are changed so that ezyhttp itself can wait on the underlying
connection to close. Please follow issue `#1925
<https://github.com/rappitz09/ezyhttp/issues/1925>`_ for the progress
on this.

HTTP Pipelining
---------------

ezyhttp does not support HTTP/HTTPS pipelining.
