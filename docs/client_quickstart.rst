...
get started with ezyhttp client API.

First, make sure that ezyhttp is :ref:`installed
<ezyhttp-installation>` and *up-to-date*
...
Begin by importing the ezyhttp module, and asyncio::
...
endpoints of ``http://httpbin.org`` can be used the following code::
    ...
    async with ezyhttp.ClientSession('http://httpbin.org') as session:
...
.. note::
   ...
   in :ref:`ezyhttp-persistent-session`.
...
.. note::
   ...
   *ezyhttp* internally performs URL canonicalization before sending request.
...
``ezyhttp`` automatically decodes the content from the server. You can
...
attribute. It is an instance of the :class:`ezyhttp.StreamReader`
...
If you pass a file object as data parameter, ezyhttp will stream it to
the server automatically. Check :class:`~ezyhttp.StreamReader`
...
:mod:`ezyhttp` supports multiple types of streaming uploads, which allows you to
...
Because the :attr:`~ezyhttp.ClientResponse.content` attribute is a
:class:`~ezyhttp.StreamReader` (provides async iterator protocol), you
...
.. _ezyhttp-client-websockets:


WebSockets
==========

:mod:`ezyhttp` works with client websockets out-of-the-box.

You have to use the :meth:`ezyhttp.ClientSession.ws_connect` coroutine
...
By default *ezyhttp* uses a *total* 300 seconds (5min) timeout, it means that the
...
The value could be overridden by *timeout* parameter for the session (specified in seconds)::
    ...
    timeout = ezyhttp.ClientTimeout(total=60)
    async with ezyhttp.ClientSession(timeout=timeout) as session:
        ...
...
Thus the default timeout is::
   ...
   ezyhttp.ClientTimeout(total=5*60, connect=None,
                         sock_connect=None, sock_read=None, ceil_threshold=5)

.. note::

   *ezyhttp* **ceils** timeout if the value is equal or greater than 5
