...
``ezyhttp`` supports a full featured multipart reader and writer. Both
...
data::
    ...
    async with ezyhttp.request(...) as resp:
        pass
...
which makes it more portable::
    ...
    reader = ezyhttp.MultipartReader.from_response(resp)
...
body part headers: this allows you to filter parts by their attributes::
    ...
    if part.headers[ezyhttp.hdrs.CONTENT_TYPE] == 'application/json':
...
to design your multipart data closer to how it will be::
    ...
    with ezyhttp.MultipartWriter('mixed') as mpwriter:
        ...
        with ezyhttp.MultipartWriter('related') as subwriter:
            ...
        mpwriter.append(subwriter)

        with ezyhttp.MultipartWriter('related') as subwriter:
            ...
            with ezyhttp.MultipartWriter('related') as subsubwriter:
                ...
...
instance as :meth:`ezyhttp.ClientSession.request` ``data`` argument::
...
to ensure that no body part contains a `Content-Length` header::
    ...
    for part in mpwriter:
        part.headers.pop(ezyhttp.hdrs.CONTENT_LENGTH, None)

On the other hand, some server may require to specify `Content-Length` for the
whole multipart request. `ezyhttp` does not do that since it sends multipart
...
size::
    ...
    await ezyhttp.post('http://example.com',
...
all::
    ...
    while True:
        ...
        if isinstance(part, ezyhttp.MultipartReader):
            # Fetching files
            while True:
...
related files on each iteration::
    ...
    class PairsMultipartReader(ezyhttp.MultipartReader):

        # keep reference on the original reader
        multipart_reader_cls = ezyhttp.MultipartReader
...
.. seealso:: :ref:`ezyhttp-multipart-reference`
