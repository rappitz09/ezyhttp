...
:mod:`ezyhttp.web` defines a set of exceptions for every *HTTP status code*.

Each exception is a subclass of :exc:`HTTPException` and relates to a single
HTTP status code::

    async def handler(request):
        raise ezyhttp.web.HTTPFound('/redirect')
...
.. exception:: HTTPException(*, headers=None, reason=None, text=None, \
   ...
   :param str reason: an optional custom HTTP reason. ezyhttp uses *default reason
