...
There are several options for ezyhttp server deployment:
...
.. _ezyhttp-deployment-standalone:

Standalone
==========

Just call :func:`ezyhttp.web.run_app` function passing
...
For running multiple ezyhttp server instances use *reverse proxies*.

.. _ezyhttp-deployment-nginx-supervisord:

Nginx+supervisord
=================

Running ezyhttp servers behind :term:`nginx` makes several advantages.

First, nginx is the perfect frontend server. It may prevent many
attacks based on malformed http protocol etc.

Second, running several ezyhttp instances behind nginx allows to
utilize all CPU cores.

Third, nginx serves static files much faster than built-in ezyhttp
static file support.
...
and redirects everything to the ``ezyhttp`` backend group.

Also it serves static files from ``/path/to/app/static`` path as
``example.com/static``.

Next we need to configure *ezyhttp upstream group*:
...
After configuring Nginx we need to start our ezyhttp backends. It's best
...
.. code-block:: cfg
   ...
   command=/path/to/ezyhttp_example.py --path=/tmp/example_%(process_num)s.sock

   ; We can just as easily pass TCP port numbers:
   ; command=/path/to/ezyhttp_example.py --port=808%(process_num)s
...
The last step is preparing the ezyhttp server to work with supervisord.

Assuming we have properly configured :class:`ezyhttp.web.Application`
...
.. code-block:: python3
   ...
   parser = argparse.ArgumentParser(description="ezyhttp server example")
...
.. _ezyhttp-deployment-gunicorn:

Nginx+Gunicorn
==============

ezyhttp can be deployed using `Gunicorn
...
<ezyhttp-deployment-nginx-supervisord>`, this solution does not need to
manually run several ezyhttp processes and use a tool like supervisord
to monitor them. But nothing is free: running ezyhttp
...
The custom worker subclass is defined in ``ezyhttp.GunicornWebWorker``::
  ...
  [2017-03-11 18:27:21 +0000] [1249] [INFO] Using worker: ezyhttp.worker.GunicornWebWorker
...
.. note::
   ...
   ``ezyhttp.GunicornUVLoopWebWorker`` worker class.
...
``ezyhttp`` and ``gunicorn`` use different format for specifying access log.

By default ezyhttp uses own defaults::

   '%a %t "%r" %s %b "%{Referer}i" "%{User-Agent}i"'

For more information please read :ref:`Format Specification for Access
Log <ezyhttp-logging-access-log-format-spec>`.


Proxy through Apache at your own risk
-------------------------------------
Issues have been reported using Apache2 in front of ezyhttp server:
`#2687 Intermittent 502 proxy errors when running behind Apache <https://github.com/rappitz09/ezyhttp/issues/2687>`.
