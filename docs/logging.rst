...
*ezyhttp* uses standard :mod:`logging` for tracking the
library activity.
...
logger ``'ezyhttp.access'`` is used, access logs will be output to
...
This logging may be controlled by :meth:`ezyhttp.web.AppRunner` and
:func:`ezyhttp.web.run_app`.
...
.. _ezyhttp-logging-access-log-format-spec:
...
Example of a drop-in replacement for the default access logger::
  ...
  from ezyhttp.abc import AbstractAccessLogger
...
:class:`ezyhttp.abc.AbstractAsyncAccessLogger`::


  from ezyhttp.abc import AbstractAsyncAccessLogger
...
:ref:`deployment <ezyhttp-deployment-gunicorn>`, its default access log format
will be automatically replaced with the default ezyhttp's access log format.

If Gunicorn's option access_logformat_ is
specified explicitly, it should use ezyhttp's format specification.
...
:mod:`ezyhttp.web` uses a logger named ``'ezyhttp.server'`` to store errors
...
instance) to the :meth:`ezyhttp.web.AppRunner` constructor.
