...
*ezyhttp* does :term:`requoting` of incoming request path.
...
:ref:`ezyhttp-web-variable-handler` please take care that URL is
*percent encoded*: if you pass Unicode patterns they don't match to
*requoted* path.

.. _ezyhttp-web-peer-disconnection:

Peer disconnection
------------------

*ezyhttp* has 2 approaches to handling client disconnections.
...
*ezyhttp* handles disconnection properly but you can handle it explicitly, e.g.::
...
internal data structures and can terminate them gracefully::
   ...
   from aiojobs.ezyhttp import setup, spawn
...
One more approach would be to use :func:`aiojobs.ezyhttp.atomic`
...
restoring the default disconnection behavior only for specific handlers::
   ...
   from aiojobs.ezyhttp import atomic
...
.. code-block:: shell
   ...
   $ gunicorn my_app_module:my_web_app --bind localhost:8080 --worker-class ezyhttp.GunicornWebWorker
...
Sometimes you need to register :ref:`handlers <ezyhttp-web-handler>` on
...
.. _ezyhttp-web-static-file-handling:
...
:mod:`ezyhttp.web` does not support template rendering out-of-the-box.

However, there is a third-party library, :mod:`ezyhttp_jinja2`, which is
supported by the *ezyhttp* authors.

Using it is rather simple. First, setup a *jinja2 environment* with a call
to :func:`ezyhttp_jinja2.setup`::

    app = web.Application()
    ezyhttp_jinja2.setup(app,
        loader=jinja2.FileSystemLoader('/path/to/templates/folder'))

After that you may use the template engine in your
:ref:`handlers <ezyhttp-web-handler>`. The most convenient way is to simply
wrap your handlers with the  :func:`ezyhttp_jinja2.template` decorator::

    @ezyhttp_jinja2.template('tmpl.jinja2')
    async def handler(request):
        return {'name': 'Andrew', 'surname': 'Svetlov'}

If you prefer the `Mako`_ template engine, please take a look at the
`ezyhttp_mako`_ library.

.. warning::

   :func:`ezyhttp_jinja2.template` should be applied **before**
...
.. _ezyhttp_mako: https://github.com/rappitz09/ezyhttp_mako


.. _ezyhttp-web-websocket-read-same-task:
...
<ezyhttp_faq_terminating_websockets>`.

:mod:`ezyhttp.web` creates an implicit :class:`asyncio.Task` for
handling every incoming request.

.. note::

   While :mod:`ezyhttp.web` itself only supports *WebSockets* without
   downgrading to *LONG-POLLING*, etc., our team supports SockJS_, an
   ezyhttp-based library for implementing SockJS-compatible server
   code.

.. _SockJS: https://github.com/rappitz09/sockjs
...
.. warning::
   ...
   See :ref:`FAQ section <ezyhttp_faq_parallel_event_sources>` for
   instructions how to solve the problem.


.. _ezyhttp-web-data-sharing:

Data Sharing aka No Singletons Please
-------------------------------------

:mod:`ezyhttp.web` discourages the use of *global variables*, aka *singletons*.
...
*ezyhttp* does not use globals by design, which will reduce the number of bugs
...
.. _ezyhttp-web-data-sharing-app-config:
...
<ezyhttp-web-nested-applications>` the desired lookup strategy could
be the following:
...
See this reference section for more detail: :ref:`ezyhttp-web-app-and-router`.
...
This is mostly useful for :ref:`ezyhttp-web-middlewares` and
:ref:`ezyhttp-web-signals` handlers to store data for further processing by the
...
To avoid clashing with other *ezyhttp* users and third-party libraries, please
...
*ezyhttp* server supports it in the following way:

* A server inherits the current task's context used when creating it.
  :func:`ezyhttp.web.run_app()` runs a task for handling all underlying jobs running
  the app, but alternatively :ref:`ezyhttp-web-app-runners` can be used.
...
* On every request handling *ezyhttp* creates a context copy. :term:`web-handler` has
...
.. _ezyhttp-web-middlewares:

Middlewares
-----------

:mod:`ezyhttp.web` provides a powerful mechanism for customizing
:ref:`request handlers<ezyhttp-web-handler>` via *middlewares*.
...
``' wink'`` to the response::
    ...
    from ezyhttp.web import middleware
...
Internally, a single :ref:`request handler <ezyhttp-web-handler>` is constructed
by applying the middleware chain to the original handler in reverse order,
and is called by the :class:`~ezyhttp.web.RequestHandler` as a regular *handler*.
...
.. _ezyhttp-web-signals:

Signals
-------

Although :ref:`middlewares <ezyhttp-web-middlewares>` can customize
:ref:`request handlers<ezyhttp-web-handler>` before or after a :class:`Response`
has been prepared, they can't customize a :class:`Response` **while** it's
being prepared. For this :mod:`ezyhttp.web` provides *signals*.
...
added. All handlers must be asynchronous since *ezyhttp* 3.0.

.. _ezyhttp-web-cleanup-ctx:
...
*ezyhttp* guarantees that *cleanup code* is called if and only if
*startup code* was successfully finished.

.. versionadded:: 3.1

.. _ezyhttp-web-nested-applications:
...
.. _ezyhttp-web-expect-header:

*Expect* Header
---------------

:mod:`ezyhttp.web` supports *Expect* header. By default it sends
...
processing application's :ref:`ezyhttp-web-middlewares` and
...
header::
   ...
   async def check_auth(request):
       if request.version != ezyhttp.HttpVersion11:
           return
...
.. _ezyhttp-web-custom-resource:

Custom resource implementation
------------------------------

To register custom resource use :meth:`~ezyhttp.web.UrlDispatcher.register_resource`.
Resource instance must implement `AbstractResource` interface.

.. _ezyhttp-web-app-runners:
...
.. _ezyhttp-web-graceful-shutdown:

Graceful shutdown
------------------

Stopping *ezyhttp web server* by just closing all connections is not
always satisfactory.

When ezyhttp is run with :func:`run_app`, it will attempt a graceful shutdown
by following these steps (if using a :ref:`runner <ezyhttp-web-app-runners>`,
...
6. Call the :attr:`Application.on_cleanup` signal. This should be used to cleanup any
   ...
   :ref:`cleanup contexts<ezyhttp-web-cleanup-ctx>`.
...
Then the signal handler may look like::
    ...
    from ezyhttp import WSCloseCode
...
.. _ezyhttp-web-ceil-absolute-timeout:

Ceil of absolute timeout value
------------------------------

*ezyhttp* **ceils** internal timeout values if the value is equal or
...
:ref:`ezyhttp-client-timeouts`.

The default threshold can be configured at :class:`ezyhttp.web.Application`
...
To run such short and long running background tasks ezyhttp provides an
...
.. _ezyhttp-web-complex-applications:

Complex Applications
^^^^^^^^^^^^^^^^^^^^

Sometimes ezyhttp is not the sole part of an application and additional
tasks/processes may need to be run alongside the ezyhttp :class:`Application`.

Generally, the best way to achieve this is to use :func:`ezyhttp.web.run_app`
...
can be done with a :ref:`ezyhttp-web-cleanup-ctx` function like::
...
by custom middleware, see :ref:`polls demo <ezyhttpdemos:ezyhttp-demos-polls-middlewares>`
for example.

.. _ezyhttp-web-forwarded-support:

Deploying behind a Proxy
------------------------

As discussed in :ref:`ezyhttp-deployment` the preferable way is
deploying *ezyhttp* web server behind a *Reverse Proxy Server* like
...
*ezyhttp* does not take *forwarded* headers into account by default
because it produces *security issue*: HTTP client might add these
headers too, pushing non-trusted data values.

That's why *ezyhttp server* should setup *forwarded* headers in custom
...
.. seealso::
   ...
   https://github.com/rappitz09/ezyhttp-remotes provides secure helpers
...
`ezyhttp-swagger <https://github.com/cr0hn/ezyhttp-swagger>`_ is a
library that allow to add Swagger documentation and embed the
Swagger-UI into your :mod:`ezyhttp.web` project.

CORS support
------------

:mod:`ezyhttp.web` itself does not support `Cross-Origin Resource
Sharing <https://en.wikipedia.org/wiki/Cross-origin_resource_sharing>`_, but
there is an ezyhttp plugin for it:
`ezyhttp_cors <https://github.com/rappitz09/ezyhttp_cors>`_.


Debug Toolbar
-------------

`ezyhttp-debugtoolbar`_ is a very useful library that provides a
debugging toolbar while you're developing an :mod:`ezyhttp.web`
application.
...
.. code-block:: shell
    ...
    $ pip install ezyhttp_debugtoolbar


Just call :func:`ezyhttp_debugtoolbar.setup`::

    import ezyhttp_debugtoolbar
    from ezyhttp_debugtoolbar import toolbar_middleware_factory

    app = web.Application()
    ezyhttp_debugtoolbar.setup(app)

The toolbar is ready to use. Enjoy!!!

.. _ezyhttp-debugtoolbar: https://github.com/rappitz09/ezyhttp_debugtoolbar


Dev Tools
---------

`ezyhttp-devtools`_ provides a couple of tools to simplify development of
:mod:`ezyhttp.web` applications.
...
* ``start`` is a `cookiecutter command which does the donkey work
  of creating new :mod:`ezyhttp.web` Applications.

Documentation and a complete tutorial of creating and running an app
locally are available at `ezyhttp-devtools`_.

.. _ezyhttp-devtools: https://github.com/rappitz09/ezyhttp-devtools
