...
ezyhttp does not support custom chunking sizes. It is up to the developer
...
ezyhttp does not enable chunked encoding automatically even if a
...
Exception hierarchy has been significantly modified. ezyhttp now defines only
exceptions that covers connection handling and server response misbehaviors.
For developer specific mistakes, ezyhttp uses python standard exceptions
...
All exceptions are moved from `ezyhttp.errors` module to top level
`ezyhttp` module.
...
1. FormData.__call__ does not take an encoding arg anymore
   ...
   ezyhttp provides payload adapters for some standard types like `str`, `byte`,
