...
.. _ezyhttp-web-lowlevel:

Low Level Server
================


This topic describes :mod:`ezyhttp.web` based *low level* API.

Abstract
--------

Sometimes users don't need high-level concepts introduced in
:ref:`ezyhttp-web`: applications, routers, middlewares and signals.

All that may be needed is supporting an asynchronous callable which accepts a
request and returns a response object.

This is done by introducing :class:`ezyhttp.web.Server` class which
...
parameter and performs one of the following actions:
  ...
  3. Raise :class:`HTTPException` derived exception (see
     :ref:`ezyhttp-web-exceptions` section).
  ...
  4. Initiate and process Web-Socket connection by
     :class:`WebSocketResponse` using (see :ref:`ezyhttp-web-websockets`).
...
:ref:`runners API <ezyhttp-web-app-runners-reference>` to serve
``http://127.0.0.1:8080/``.
