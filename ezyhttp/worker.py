"""Async gunicorn worker for ezyhttp.web"""
...
class GunicornWebWorker(base.Worker):  # type: ignore[misc,no-any-unimported]
    DEFAULT_ezyhttp_LOG_FORMAT = AccessLogger.LOG_FORMAT
    ...
    def init_signals(self) -> None:
        ...
        # See: https://github.com/rappitz09/ezyhttp/issues/6130
    ...
    def _get_valid_log_format(self, source_format: str) -> str:
        if source_format == self.DEFAULT_GUNICORN_LOG_FORMAT:
            return self.DEFAULT_ezyhttp_LOG_FORMAT
        elif re.search(r"%\([^\)]+\)", source_format):
            raise ValueError(
                "Gunicorn's style options in form of `%(name)s` are not "
                "supported for the log formatting. Please use ezyhttp's "
                "format specification to configure access log formatting: "
                "http://docs.ezyhttp.org/en/stable/logging.html"
