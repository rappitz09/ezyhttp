...
ezyhttpClient = Callable[[Union[Application, BaseTestServer]], Awaitable[TestClient]]
ezyhttpRawServer = Callable[[Application], Awaitable[RawTestServer]]
ezyhttpServer = Callable[[Application], Awaitable[TestServer]]
...
def fast(request):  # type: ignore[no-untyped-def]
    """--fast config option"""
    return request.config.getoption("--ezyhttp-fast")


@pytest.fixture
def loop_debug(request):  # type: ignore[no-untyped-def]
    """--enable-loop-debug config option"""
    return request.config.getoption("--ezyhttp-enable-loop-debug")
...
def pytest_pyfunc_call(pyfuncitem):  # type: ignore[no-untyped-def]
    ...
    fast = pyfuncitem.config.getoption("--ezyhttp-fast")
...
def pytest_generate_tests(metafunc):  # type: ignore[no-untyped-def]
    ...
    loops = metafunc.config.option.ezyhttp_loop
...
@pytest.fixture
def ezyhttp_unused_port() -> Callable[[], int]:
    """Return a port that is unused on the current host."""
    return _unused_port


@pytest.fixture
def ezyhttp_server(loop: asyncio.AbstractEventLoop) -> Iterator[ezyhttpServer]:
    """Factory to create a TestServer instance, given an app.

    ezyhttp_server(app, **kwargs)
    """
    servers = []
...
@pytest.fixture
def ezyhttp_raw_server(loop: asyncio.AbstractEventLoop) -> Iterator[ezyhttpRawServer]:
    """Factory to create a RawTestServer instance, given a web handler.

    ezyhttp_raw_server(handler, **kwargs)
    """
    servers = []
...
@pytest.fixture
def ezyhttp_client_cls() -> Type[TestClient]:
    """
    Client class to use in ``ezyhttp_client`` factory.
...
@pytest.fixture
def ezyhttp_client(
    loop: asyncio.AbstractEventLoop, ezyhttp_client_cls: Type[TestClient]
) -> Iterator[ezyhttpClient]:
    """Factory to create a TestClient instance.

    ezyhttp_client(app, **kwargs)
    ...
    async def go(
        ...
        **kwargs: Any
        ...
        if isinstance(__param, Application):
            ...
            client = ezyhttp_client_cls(server, **kwargs)
        elif isinstance(__param, BaseTestServer):
            client = ezyhttp_client_cls(__param, **kwargs)
        else:
