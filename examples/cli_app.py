#!/usr/bin/env python3
"""
Example of serving an Application using the `ezyhttp.web` CLI.

Serve this app using::

    $ python -m ezyhttp.web -H localhost -P 8080 --repeat 10 cli_app:init \
    > "Hello World"

Here ``--repeat`` & ``"Hello World"`` are application specific command-line
arguments. `ezyhttp.web` only parses & consumes the command-line arguments it
...
def init(argv: Optional[Sequence[str]]) -> web.Application:
    arg_parser = ArgumentParser(
        prog="ezyhttp.web ...", description="Application CLI", add_help=False
    )
    ...
    # Avoid conflict with -h from `ezyhttp.web` CLI parser
    arg_parser.add_argument(
