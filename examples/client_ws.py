...
async def start_client(url: str) -> None:
    ...
    async def dispatch(ws: ezyhttp.ClientWebSocketResponse) -> None:
        while True:
            msg = await ws.receive()

            if msg.type == ezyhttp.WSMsgType.TEXT:
                print("Text: ", msg.data.strip())
            elif msg.type == ezyhttp.WSMsgType.BINARY:
                print("Binary: ", msg.data)
            elif msg.type == ezyhttp.WSMsgType.PING:
                await ws.pong()
            elif msg.type == ezyhttp.WSMsgType.PONG:
                print("Pong received")
            else:
                if msg.type == ezyhttp.WSMsgType.CLOSE:
                    await ws.close()
                elif msg.type == ezyhttp.WSMsgType.ERROR:
                    print("Error during receive %s" % ws.exception())
                elif msg.type == ezyhttp.WSMsgType.CLOSED:
                    pass
